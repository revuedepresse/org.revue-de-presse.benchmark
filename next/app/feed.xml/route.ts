...
// description<-text plain, content<-<img/> + linkified HTML in
// <content:encoded>; avatar is inlined at 48×48 to match the on-site
// Bluesky post card), same yesterday-only window, same swallow-on-error
// behaviour (emit channel metadata with zero items rather than 5xx). Uses
// the `feed` library — the same one nuxt-module-feed wraps internally — so
// RSS escaping/structure matches the Nuxt build.
//
