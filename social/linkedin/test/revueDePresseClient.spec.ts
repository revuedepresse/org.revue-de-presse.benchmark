...
describe('revueDePresseClient', () => {
  it('mints a token then fetches highlights for the given date', async () => {
    ...
    const out = await client.fetchTopTen('2026-05-20');
  ...
  it('reuses a cached token across calls', async () => {
    ...
    await client.fetchTopTen('2026-05-20');
    await client.fetchTopTen('2026-05-21');
  ...
  it('throws UpstreamError on 5xx', async () => {
    ...
    await expect(client.fetchTopTen('2026-05-20')).rejects.toBeInstanceOf(UpstreamError);
  });
});
