#!/usr/bin/env node
// After `mitosis build`:
//  1. Copy shared modules (utils, locales, types) into output/vue/src/ so the
//     emitted components can resolve relative imports like `../utils/i18n`.
//  2. Patch Vue's inline-style emit and single-interpolation <template> blocks
//     (see in-line comments below) — both work around Mitosis 0.13 quirks.
//  3. Collect per-component CSS from .lite.tsx sources into output/components.css
//     for the Nuxt app to import once.
// Idempotent.
...
// stylesheet imported by the Nuxt app alongside tokens.css.
