...
type Segment = { kind: 'text' | 'mention' | 'url'; value: string; href: string };
...
export default function BlueskyPostCard(props: BlueskyPostCardProps) {
  const state = useStore({
    ...
    get publicationUrl(): string {
      return props.post.publicationUrl ?? `https://bsky.app/profile/${props.post.authorHandle}`;
    },
    get bodySegments(): Segment[] {
      ...
      while (m !== null) {
        if (m.index > last) {
          out.push({ kind: 'text', value: text.slice(last, m.index), href: '' });
        }
        const matched = m[0];
        if (matched.charAt(0) === '@') {
          ...
          if (trailing > 0) {
            out.push({ kind: 'text', value: matched.slice(matched.length - trailing), href: '' });
          }
        } else if (matched.charAt(0) === 'h') {
          const trimmed = matched.replace(/[.,;:!?)]+$/, '');
          const trailing = matched.length - trimmed.length;
          out.push({ kind: 'url', value: trimmed, href: trimmed });
          if (trailing > 0) {
            out.push({ kind: 'text', value: matched.slice(matched.length - trailing), href: '' });
          }
        } else {
          const trimmed = matched.replace(/[.,;:!?)]+$/, '');
          const trailing = matched.length - trimmed.length;
          out.push({ kind: 'url', value: trimmed, href: `https://${trimmed}` });
          if (trailing > 0) {
            out.push({ kind: 'text', value: matched.slice(matched.length - trailing), href: '' });
          }
        }
      ...
      if (last < text.length) {
        out.push({ kind: 'text', value: text.slice(last), href: '' });
      }
      return out;
    },
  });
    ...
    <div class="rdp-bsky-post-frame">
      ...
      <article class="rdp-bsky-post" data-testid="post-card">
        <a
          ...
          aria-label="Bluesky"
          href={state.publicationUrl}
          rel="noreferrer nofollow noopener"
          target="_blank"
        >
          <svg viewBox="0 0 600 530" width="20" height="20" fill="currentColor" aria-hidden="true">
        ...
        <p class="rdp-bsky-post__body">
          <For each={state.bodySegments}>
            {(seg) => (
              <>
                <Show when={seg.kind === 'text'}>{seg.value}</Show>
                <Show when={seg.kind === 'mention'}>
                  <a
