<template>
  <article class="rdp-terms-of-service">
    ...
    <p>En vigueur au {{ effectiveAt ?? "27/05/26" }}</p>
    <p>
      Les présentes conditions générales d'utilisation (les «&nbsp;CGU&nbsp;»)
      régissent l'accès au site {{ site ?? "revue-de-presse.org" }}, ci-après le
      «&nbsp;Site&nbsp;», ainsi qu'à l'ensemble des services associés exposés
      par {{ editor ?? "Thierry Marianne" }}, ci-après l'«&nbsp;Éditeur&nbsp;»,
      joignable à l'adresse&nbsp;
      <a :href="`mailto:${email ?? 'contact@revue-de-presse.org'}`">{{
        email ?? "contact@revue-de-presse.org"
      }}</a
    ...
    <p>
      ...
      complémentaires&nbsp;: archivage par date, consultation des sources, et
      une synthèse quotidienne. L'ensemble des contenus est publié à titre
      d'information et sans valeur contractuelle.
    </p>
    <p>
      La synthèse quotidienne est générée automatiquement à partir des dix
      publications retenues, par un modèle de langage open-source<strong
        >Mistral 7B Instruct v0.3</strong
      >
      (quantification Q5_K_M, exécuté en local sur l'infrastructure de l'Éditeur
      — aucun appel à un service d'inférence tiers, aucune donnée transmise hors
      du Site). Le modèle peut, comme tout LLM, produire des
      inexactitudes&nbsp;; le texte généré est publié à titre indicatif et ne
      saurait engager l'Éditeur. Les liens vers les comptes Bluesky cités dans
      la synthèse sont restreints à une liste d'éditeurs de presse vérifiés afin
    ...
    <p>
      ...
      responsable des interruptions, ni de toute conséquence pouvant en résulter
      pour l'Utilisateur ou tout tiers.
    </p>
    <h2>ARTICLE 3 – COMPTES DE DIFFUSION SUR LES RÉSEAUX SOCIAUX</h2>
    <p>
      Afin de diffuser la sélection quotidienne au-delà du Site, l'Éditeur opère
      plusieurs comptes officiels sur les principaux réseaux sociaux,
      notamment&nbsp;:
    </p>
    <ul>
      <li>
      ...
      <li>
        TikTok&nbsp;:<a
          ...
          :href="`https://www.tiktok.com/@${(
            tiktokAccount ?? 'revue_2_presse'
          ).replace(/^@/, '')}`"
        >
          @{{ (tiktokAccount ?? "revue_2_presse").replace(/^@/, "") }}</a
        >
        .
      </li>
    </ul>
    ...
    <p>
      L'Éditeur exploite une application TikTok déclarée auprès de<a
    ...
    <ul>
      <li>
        ...
        informations publiques de profil du compte connecté (identifiant opaque,
        nom affiché, image d'avatar). Ces informations servent uniquement à
        confirmer que l'autorisation a bien été délivrée pour le compte attendu.
        Elles ne sont ni revendues, ni rapprochées d'autres jeux de données.
      </li>
      <li>
        ...
        compte officiel pour validation manuelle, ou publication directe lorsque
        l'application est habilitée.
      </li>
      <li>
    ...
    <p>
      ...
      interdits&nbsp;: la collecte automatisée de contenus en dehors des canaux
      prévus (RSS, sitemap), le contournement des quotas d'usage de l'assistant
      de discussion, l'envoi massif de requêtes, la sollicitation de contenus
      illicites, diffamatoires, haineux ou portant atteinte aux droits de tiers.
      L'Éditeur se réserve le droit de suspendre l'accès, sans préavis, en cas
      d'usage manifestement abusif.
    </p>
    <h2>ARTICLE 6 – PROPRIÉTÉ INTELLECTUELLE</h2>
    <p>
      L'ensemble des éléments éditoriaux propres au Site (mise en page,
      rédactionnels, identité visuelle) est protégé par le droit de la propriété
      intellectuelle. Les extraits de presse repris par le Site restent la
      propriété de leurs éditeurs respectifs et sont reproduits à titre de
      courte citation, conformément à l'article L.122-5 du Code de la propriété
    ...
    <p>
      ...
      personnelles, y compris celles liées à l'intégration TikTok, sont décrites
      dans la<a href="/mentions-legales#confidentialite">
    ...
    <p>
      ...
      moment. La version applicable est celle en vigueur à la date de l'accès au
      Site, dont la date d'effet figure en tête du présent document.
    </p>
...
const props = defineProps<TermsOfServicePageProps>();
</script>