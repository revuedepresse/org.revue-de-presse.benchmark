  ...
  template: `
    <article class="rdp-terms-of-service">
      ...
      <p>
        ...
        le «&nbsp;Site&nbsp;», ainsi qu'à l'ensemble des services associés
        exposés par {{editor ?? 'Thierry Marianne'}} , ci-après
      ...
      <p>
        ...
        fonctionnalités complémentaires&nbsp;: archivage par date, consultation
        des sources, et une synthèse quotidienne. L'ensemble des contenus est
      ...
      <p>
        ...
        la synthèse sont restreints à une liste d'éditeurs de presse vérifiés
        afin d'éviter toute redirection erronée.
      </p>
      ...
      <ul>
        ...
        <li>
          ...
          <a
            rel="noreferrer nofollow noopener"
            target="_blank"
            [attr.href]="\`https://www.tiktok.com/@\${(tiktokAccount ?? 'revue_2_presse').replace(/^@/, '')}\`"
          >
      ...
      <ul>
        <li>
          ...
          compte connecté (identifiant opaque, nom affiché, image d'avatar). Ces
          informations servent uniquement à confirmer que l'autorisation a bien
          été délivrée pour le compte attendu. Elles ne sont ni revendues, ni
          rapprochées d'autres jeux de données.
        </li>
        <li>
          <strong>video.upload</strong>
          (Content Posting API)&nbsp;: dépôt de la vidéo quotidienne en tant que
          brouillon dans la boîte de réception du compte officiel pour
          validation manuelle, ou publication directe lorsque l'application est
          habilitée.
        </li>
        <li>
          <strong>video.list</strong>
          (Content Posting API)&nbsp;: lecture de la liste des vidéos publiques
          du compte officiel afin de vérifier le bon déroulement des
...
export default class TermsOfServicePage {
  @Input() effectiveAt!: TermsOfServicePageProps["effectiveAt"];
  @Input() site!: TermsOfServicePageProps["site"];
  @Input() editor!: TermsOfServicePageProps["editor"];
  @Input() email!: TermsOfServicePageProps["email"];
  @Input() tiktokAccount!: TermsOfServicePageProps["tiktokAccount"];
}
