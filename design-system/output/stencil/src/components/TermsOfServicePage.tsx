...
@Component({
  tag: "terms-of-service-page",
})
export class TermsOfServicePage {
  @Prop() effectiveAt: any;
  @Prop() site: any;
  @Prop() editor: any;
  @Prop() email: any;
  @Prop() tiktokAccount: any;
  ...
  render() {
    return (
      <article class="rdp-terms-of-service">
        ...
        <p>
          En vigueur au
          {this.effectiveAt ?? "27/05/26"}
        </p>
        <p>
          Les présentes conditions générales d'utilisation (les
          «&nbsp;CGU&nbsp;») régissent l'accès au site
          {this.site ?? "revue-de-presse.org"}, ci-après le «&nbsp;Site&nbsp;»,
          ainsi qu'à l'ensemble des services associés exposés par
          {this.editor ?? "Thierry Marianne"}, ci-après l'«&nbsp;Éditeur&nbsp;»,
          joignable à l'adresse&nbsp;
          <a href={`mailto:${this.email ?? "contact@revue-de-presse.org"}`}>
            {this.email ?? "contact@revue-de-presse.org"}
          </a>
          . La navigation sur le Site et l'usage des services associés
        ...
        <p>
          ...
          fonctionnalités complémentaires&nbsp;: archivage par date,
          consultation des sources, et un assistant de discussion en langage
          naturel. L'ensemble des contenus est publié à titre d'information et
        ...
        <p>
          ...
          majeure ou interruption pour maintenance. L'Éditeur ne saurait être
          tenu responsable des interruptions, ni de toute conséquence pouvant en
          ...
          <li>
            ...
            <a
              ...
              href={`https://www.tiktok.com/@${(
                this.tiktokAccount ?? "revue_2_presse"
              ).replace(/^@/, "")}`}
            >
              @{(this.tiktokAccount ?? "revue_2_presse").replace(/^@/, "")}
            </a>
            .
          </li>
        </ul>
        <p>
          Ces comptes sont exclusivement administrés par l'Éditeur pour son
          propre compte. Aucune fonctionnalité du Site ne permet à un
          Utilisateur tiers de publier sur ces comptes, ni de connecter son
          propre compte TikTok à revue-de-presse.org.
        </p>
        ...
        <p>
          ...
          («&nbsp;scopes&nbsp;») suivantes, et uniquement pour le compte
          officiel @{(this.tiktokAccount ?? "revue_2_presse").replace(/^@/, "")}
          &nbsp;:
        </p>
        <ul>
          <li>
            <strong>user.info.basic</strong>
            (Login Kit)&nbsp;: lecture des informations publiques de profil du
            compte connecté (identifiant opaque, nom affiché, image d'avatar).
            Ces informations servent uniquement à confirmer que l'autorisation a
            bien été délivrée pour le compte attendu. Elles ne sont ni
            revendues, ni rapprochées d'autres jeux de données.
          </li>
          <li>
            <strong>video.upload</strong>
            (Content Posting API)&nbsp;: dépôt de la vidéo quotidienne en tant
            que brouillon dans la boîte de réception du compte officiel pour
            validation manuelle, ou publication directe lorsque l'application
            est habilitée.
          </li>
          <li>
            <strong>video.list</strong>
            (Content Posting API)&nbsp;: lecture de la liste des vidéos
            publiques du compte officiel afin de vérifier le bon déroulement des
            publications quotidiennes et d'éviter les doublons.
          </li>
        </ul>
        <p>
          Aucun de ces accès n'est utilisé pour collecter ou afficher des
          données relatives à des Utilisateurs tiers. L'Éditeur ne stocke ni
          n'expose l'identifiant TikTok, l'image d'avatar ou le nom affiché du
          compte officiel sur le Site&nbsp;; ces informations ne servent qu'à
        ...
        <p>
          ...
          l'assistant de discussion, l'envoi massif de requêtes, la
          sollicitation de contenus illicites, diffamatoires, haineux ou portant
          atteinte aux droits de tiers. L'Éditeur se réserve le droit de
          suspendre l'accès, sans préavis, en cas d'usage manifestement abusif.
        </p>
        ...
        <p>
          ...
          l'accès au Site, dont la date d'effet figure en tête du présent
          document.
        </p>
