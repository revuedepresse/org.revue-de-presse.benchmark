...
Design system for [revue-de-presse.org](https://revue-de-presse.org), authored once in [Mitosis](https://github.com/BuilderIO/mitosis) and compiled to Vue for the Nuxt app.
...
- `output/vue/` — Mitosis emit (gitignored at `output/*/dist`). After Mitosis runs, `scripts/post-mitosis.mjs` hydrates the target with `src/utils`, `src/locales`, and `src/types.ts` so emitted relative imports resolve.
